...
# ---------------------------------------------------------------------------
# Exception bridge — plain object E with no ``toException``. Wrapped in
# ResultError[E] with the stringified payload as message: system's generic
# object ``$`` applies, so the ``compiles($e)`` branch is taken, and this
# must hold for strict and non-strict consumers alike.
# ---------------------------------------------------------------------------
block opaqueErr:
  type Opaque = object
    dummy: int
